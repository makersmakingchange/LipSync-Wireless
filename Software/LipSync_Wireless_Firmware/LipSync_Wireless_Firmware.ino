...
void moveBluetoothCursor(const int buttonCursor,const int xCursor,const int yCursor,const int wheel) {
  ...
  bluetoothPacket[3] = lowByte(buttonCursor);
  bluetoothPacket[4] = lowByte(xCursor);
  bluetoothPacket[5] = lowByte(yCursor);
  bluetoothPacket[6] = lowByte(wheel);
      ...
      case OUTPUT_SECONDARY_SCROLL:
        {
          ...
          // Default: if sip counter value is under 750 and more than SIP_COUNT_THRESHOLD_MED ( 3 Second Long Sip )
          cursorSecondaryScroll(modeAction); // Enter Scroll mode
          delay(5);
          break;
        }
    }// end switch
...
void cursorDrag(int mode)
{
  if (mode == 0) {
    ...
    if (g_cursorClickStatus == 0) {
      //ledOn(2);
      g_cursorClickStatus = 1;
    } else if (g_cursorClickStatus == 1) {
      ledClear();
      g_cursorClickStatus = 0;
    }    
  }
