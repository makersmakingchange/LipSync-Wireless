...
//
//  ++        ++   ++   ++++++   ++++++   ++       ++++++     ++++     ++++
//  oo        oo   oo   oo""oo   oooooo   oo       oooooo   .oo"Yo   .oo"Yo 
//   oo  db  oo    oo   oo__oo   oo__     oo       oo__     `ooo."   `ooo." 
//    oooooooo     oo   oo"oo    oo""     oo  .o   oo""       o.`Yob     o.`Yob 
//     oo  oo      oo   oo  oo   oooooo   oooodo   oooooo     obooo'     obooo' 
//
...
#define BT_CONFIG_NUMBER 3						  //Bluetooth Config number for LipSync Wireless
...
void setup() {
  ...
  delay(50); 
  commMode = getCommunicationMode(false);         //Identify the communication mode ( Bluetooth or USB )
  delay(10);
  setBluetoothConfig(false);                      //Get bluetooth configure number and reconfigure if it is needed
  delay(10);
  ...
  forceCursorDisplay(commMode);                           //Display cursor on screen by moving it
  ...
  if ((xHighYHigh > xHighYHighRadius) || (xHighYLow > xHighYLowRadius) || (xLowYLow > xLowYLowRadius) || (xLowYHigh > xLowYHighRadius)) {
    ...
    if (pollCounter >= 3) {
      ...
      if (commMode == 0) {
		...
		if ((xHighYHigh >= xHighYLow) && (xHighYHigh >= xLowYHigh) && (xHighYHigh >= xLowYLow)) {
          //Serial.println("quad1");
          (rawModeEnabled)? sendRawData(xCursorHigh(xHigh),yCursorHigh(yHigh),sipAndPuffRawHandler(),xHigh,xLow,yHigh,yLow) : sendBluetoothCommand(cursorClickStatus, xCursorHigh(xHigh), yCursorHigh(yHigh), 0);
          delay(cursorDelay);
          pollCounter = 0;
        } else if ((xHighYLow > xHighYHigh) && (xHighYLow > xLowYLow) && (xHighYLow > xLowYHigh)) {
          //Serial.println("quad4");
          (rawModeEnabled)? sendRawData(xCursorHigh(xHigh),yCursorLow(yLow),sipAndPuffRawHandler(),xHigh,xLow,yHigh,yLow) : sendBluetoothCommand(cursorClickStatus, xCursorHigh(xHigh), yCursorLow(yLow), 0);
          delay(cursorDelay);
          pollCounter = 0;
        } else if ((xLowYLow >= xHighYHigh) && (xLowYLow >= xHighYLow) && (xLowYLow >= xLowYHigh)) {
          //Serial.println("quad3");
          (rawModeEnabled)? sendRawData(xCursorLow(xLow),yCursorLow(yLow),sipAndPuffRawHandler(),xHigh,xLow,yHigh,yLow) : sendBluetoothCommand(cursorClickStatus, xCursorLow(xLow), yCursorLow(yLow), 0);
          delay(cursorDelay);
          pollCounter = 0;
        } else if ((xLowYHigh > xHighYHigh) && (xLowYHigh >= xHighYLow) && (xLowYHigh >= xLowYLow)) {
          //Serial.println("quad2");
          (rawModeEnabled)? sendRawData(xCursorLow(xLow),yCursorHigh(yHigh),sipAndPuffRawHandler(),xHigh,xLow,yHigh,yLow) : sendBluetoothCommand(cursorClickStatus, xCursorLow(xLow), yCursorHigh(yHigh), 0);
          delay(cursorDelay);
          pollCounter = 0;
        }
	  }
...
void getModelNumber(bool responseEnabled) {
  ...
  if (modelNumber != 3) {                                 //If the previous firmware was different model then factory reset the settings 
    modelNumber = 3;                                      //And store the model number in EEPROM 
...
int increaseCursorSpeed(int speedCounter,bool responseEnabled) {
  ...
  if (speedCounter == 11) {
    ...
    cursorMaxBTSpeed = cursorParams[speedCounter]._maxBTSpeed;
...
void sipAndPuffHandler(int mode) {
  ...
  if (cursorPressure < puffThreshold) {             
    ...
    //Puff actions 
      if (puffCount < 150) {
        performButtonAction(mode, actionButton[0]);
      } else if (puffCount > 150 && puffCount < 750) {
        performButtonAction(mode, actionButton[2]);
      } else if (puffCount > 750) {
        performButtonAction(mode, actionButton[4]);
      }
  ...
  if (cursorPressure > sipThreshold) {
    ...
    //Sip actions 
      if (sipCount < 150) {
        performButtonAction(mode, actionButton[1]);
      } else if (sipCount > 150 && sipCount < 750) {
        performButtonAction(mode, actionButton[3]);
      } else {
        //Perform seconday function if sip counter value is more than 750 ( 5 second Long Sip )
        performButtonAction(mode, actionButton[5]);
      }
...
void clearButtonAction(int mode){
  ledClear();
  if(mode == 0){
...
void performButtonAction(int mode, int actionButtonNumber) {
...
void forceCursorDisplay(int mode) {
  if (mode == 0) {
    Mouse.move(1, 0, 0);
    delay(25);
    Mouse.move(-1, 0, 0);
...
void cursorLeftClick(int mode) {
	...
	if (mode == 0) {
		...
		sendBluetoothCommand(cursorClickStatus, 0, 0, 0);
		clearBluetoothCommand();
		cursorClickStatus = 0;
	}
  delay(5);
...
void cursorRightClick(int mode) {
	ledClear();
	if (mode == 0) {
...
void cursorSwipe(int mode) {
  if (mode == 0) {
    ...
    cursorClickStatus = 0;
    for (int i = 0; i < 3; i++) sendBluetoothCommand(cursorClickStatus, 0, 126, 0);
    delay(125);

    cursorClickStatus = 1;
    for (int j = 0; j < 3; j++) sendBluetoothCommand(cursorClickStatus, 0, -126, 0);
    clearBluetoothCommand();
    cursorClickStatus = 0;
    delay(125);
  }
...
void cursorMiddleClick(int mode) {
	ledClear();
	if (mode == 0) {
...
void cursorScroll(int mode) {
  ...
  while (1) {
    ...
    if (mode == 0) {
      ...
      if (scrollUp > yHighNeutral + 30) {
        sendBluetoothCommand(0, 0, 0, -1 * yCursorHigh(scrollUp));
        delay(cursorDelay * 35);
      } else if (scrollDown > yLowNeutral + 30) {
        sendBluetoothCommand(0, 0, 0, -1 * yCursorLow(scrollDown));
        delay(cursorDelay * 35);
      } else if ((scrollRelease > sipThreshold) || (scrollRelease < puffThreshold)) {
        break;
      }
...
void sendBluetoothCommand(int buttons, int x, int y, int scroll) {
...
int getCommunicationMode(bool responseEnabled) {
	int mode =0;
...
int setCommunicationMode(int mode, bool responseEnabled) {
  if(responseEnabled) {
    Serial.print("SUCCESS:CM,1:");
    Serial.println(mode); 
    delay(5);
   }
  return mode;
